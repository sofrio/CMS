...
SENSOR_TRIGGER = len(sys.argv) < 2 or sys.argv[1] != "-c"
RUN_MODE = "センサー" if SENSOR_TRIGGER else "常時録画"

UPLOAD_META_DATA = True
UPLOAD_IMAGE = True
UPLOAD_SOUND = True
TRACE_UPLOAD = True
...
try:
    CAMERA_ID, LAT, LON = ut.file_get_contents("cameraID").split()
...
BUFFER_DIR = ut.TMP_DIR
BUFFER_NAME = BUFFER_DIR + "buf"
BUFFER_SIZE = 60 if SENSOR_TRIGGER else 30
MAX_THREADING_FILES = BUFFER_SIZE
ADJUST_CYCLE = 4
...
class Buffer():
    ...
    def upload(self, camera):
        ...
        for i in range(len(os.listdir(self.dir))):
            ...
            if Buffer.file_count() > MAX_THREADING_FILES:
                camera.upload_file(self.url, path, cap_time)
            else:
        ...
        if files == 0 or (self.remains > 0 and abs(adjust) >= FPS_ADJUST):
            ...
            if new_fps != fps:
                if new_fps < fps:
                    ...
                    self.adjusts += 1
                    if self.adjusts % ADJUST_CYCLE == 0:
